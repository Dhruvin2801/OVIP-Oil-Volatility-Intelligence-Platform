...
<style>
    ...
    /* Custom Scrollbar for Intel logs */
        ...
        if os.path.exists(f): 
            try:
                ...
                if 'Score' not in df.columns:
                    df['Score'] = np.random.normal(0, 1, len(df)) # Fallback sentiment
                return df
            except Exception: pass
    
    dates = pd.date_range(end=pd.Timestamp.today(), periods=150)
    t = np.linspace(0, 4*np.pi, 150)
    score = np.sin(t) + np.random.normal(0, 0.5, 150) # Simulated wave sentiment
    return pd.DataFrame({'Date': dates, 'WTI': np.random.normal(75, 2, 150), 'Volatility': np.random.normal(0.15, 0.02, 150), 'Score': score, 'gpr': np.random.rand(150)*100})
...
# Auto-fill missing specific intel for the rest of the nodes so no code breaks
for k in ['UAE', 'VENEZUELA', 'BRAZIL', 'UK', 'NORWAY', 'NIGERIA', 'ANGOLA', 'LIBYA', 'IRAQ', 'KUWAIT', 'QATAR', 'CANADA', 'MEXICO', 'GERMANY', 'JAPAN', 'SOUTH KOREA', 'AUSTRALIA', 'ALGERIA', 'EGYPT', 'TURKEY', 'SOUTH AFRICA', 'SINGAPORE', 'INDONESIA', 'OMAN']:
    if k not in COUNTRIES:
        COUNTRIES[k] = {'lat': 0, 'lon': 0, 'risk': 'MEDIUM', 'color': C_WARN, 'mod': 1.0, 'info': 'Generic node tracking.', 'catalyst': 'Global macro headwinds.', 'vuln': 'Systemic supply chain exposure.'}

# Fix actual coordinates for visual filler
...
if st.session_state.target is None:
    # --- GLOBE VIEW (SPLIT SCREEN NO EMPTY SPACE) ---
    c_globe, c_intel = st.columns([6.5, 3.5])
    ...
    with c_intel:
        # Panel 1: Live Event Impact
        st.markdown("<div class='animated-panel' style='height: 250px; overflow-y: auto;'>", unsafe_allow_html=True)
        st.markdown("<h4 style='border-bottom: 1px solid rgba(0,240,255,0.3); padding-bottom: 5px;'>MACRO_EVENT_IMPACT</h4>", unsafe_allow_html=True)
        st.markdown("""
        <div style='font-family: JetBrains Mono; font-size: 13px; color: #cbd5e1; line-height: 1.5; margin-top: 10px;'>
            <span style='color: #ff003c; font-weight: bold;'>[CRITICAL] U.S. TARIFF ENACTMENT:</span> Broad-based global tariffs triggered immediate supply chain rerouting. WTI transport premiums spiked 14%. Volatility accelerating.<br><br>
            <span style='color: #ffd700; font-weight: bold;'>[WARNING] OPEC+ QUOTA STRICTNESS:</span> Sudden restriction of physical market liquidity. Hard floor established on crude pricing, suppressing downside action.<br><br>
            <span style='color: #00f0ff; font-weight: bold;'>[UPDATE] RED SEA TRANSIT:</span> Rerouting via Cape of Good Hope adding 14 days to East-West freight. Tanker utilization at 98%.
        </div>
        """, unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

        # Panel 2: Sentiment Graph
        st.markdown("<div class='animated-panel' style='height: 200px;'>", unsafe_allow_html=True)
        st.markdown("<h4 style='margin-bottom: 5px;'>NLP_SENTIMENT_INDEX</h4>", unsafe_allow_html=True)
        st.plotly_chart(create_sentiment_chart(df_main.tail(60)), use_container_width=True, config={'displayModeBar': False})
        st.markdown("</div>", unsafe_allow_html=True)
        
        # Panel 3: Global Metrics
        st.markdown("<div class='animated-panel' style='height: 160px;'>", unsafe_allow_html=True)
        m1, m2 = st.columns(2)
        m1.metric("GLOBAL WTI INDEX", f"${df_main.iloc[-1]['WTI']:.2f}")
        m2.metric("MACRO VOLATILITY", f"{df_main.iloc[-1]['Volatility']:.3f}")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"<p style='color:#94a3b8; font-family: JetBrains Mono; font-size: 14px; margin-top: -15px;'>> UPLINK_ESTABLISHED: <span style='color:{intel['color']}; font-weight:bold;'>{target}</span></p>", unsafe_allow_html=True)
    if st.button("← RETURN TO GLOBE OVERVIEW"): st.session_state.target = None; st.rerun()
    ...
    with c_sent:
        ...
        # Shift sentiment locally based on mod
    ...
    with col_chart:
        ...
        # Explicit Explanation of the Volatility Graph
        st.markdown("""
