...
def load_data():
    ...
    return pd.DataFrame({'Date': dates, 'WTI': np.random.normal(75, 2, 150), 'Volatility': np.random.normal(0.15, 0.02, 150), 'Crisis_Prob': np.random.uniform(0.1, 0.9, 150), 'gpr': np.random.rand(150)*100})
...
def create_gauge(val, color):
    fig = go.Figure(go.Indicator(
        mode = "gauge+number", value = val * 100,
        number = {'suffix': "%", 'font': {'color': color, 'family': 'JetBrains Mono', 'size': 18}},
        title = {'text': "CRISIS_PROBABILITY", 'font': {'color': '#94a3b8', 'family': 'Orbitron', 'size': 10}},
        gauge = {'axis': {'range': [None, 100], 'tickwidth': 1, 'tickcolor': "rgba(255,255,255,0.2)"},
                 'bar': {'color': color}, 'bgcolor': "rgba(0,0,0,0)", 'borderwidth': 1, 'bordercolor': "rgba(0, 240, 255, 0.3)"}
    ))
    fig.update_layout(height=120, margin=dict(l=10, r=10, t=30, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    return fig
...
if st.session_state.target is None:
    ...
    with col_chart:
        ...
        # ADDED VISUAL: Crisis Probability Gauge
        st.plotly_chart(create_gauge(latest.get('Crisis_Prob', 0.5) * mod, intel['color']), use_container_width=True, config={'displayModeBar': False})
    ...
    with col_intel:
        ...
        # SPECIFIC DETAILS SECTION (NO EMOJIS, NO BLANK BOXES)
        st.markdown(f"""
        ...
        <div>
            <p style='color: #00f0ff; font-weight: 700; font-size: 1em; font-family: "Orbitron"; margin-bottom: 5px;'>[ MARKET_DATA ]</p>
            <p style='font-family: "JetBrains Mono"; font-size: 13px; line-height: 1.5; color: #94a3b8;'>> {intel['figures']}</p>
        </div>
        
        <div style='margin-top: 15px;'>
            <p style='color: {intel["color"]}; font-weight: 700; font-size: 1em; font-family: "Orbitron"; margin-bottom: 5px;'>[ EVENT_IMPACT ]</p>
            <p style='font-family: "JetBrains Mono"; font-size: 13px; line-height: 1.5; color: #94a3b8;'>> {intel['impact']}</p>
        </div>
